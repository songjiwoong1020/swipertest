import { DEFAULT_ERROR_MESSAGE, FETCH_ERROR_MESSAGE } from '@/constants/errorMessage';
import { ErrorSeverity, ErrorType, logError } from '@/utils/errorLogger';
import { isBlobResponse, isEmptyResponse, isErrorWithStatus, isSuccess } from '@/utils/fetchUtils';
...
export const apiClient = {
  ...
  delete: <Response>(URI: string, options?: Omit<ApiClientProps<undefined>, 'method' | 'URI'>) =>
...
async function baseClient<Response, RequestBody>({
  ...
  try {
    ...
    if (isErrorWithStatus(response)) {
      let message = FETCH_ERROR_MESSAGE[String(response.status)] ?? DEFAULT_ERROR_MESSAGE;
